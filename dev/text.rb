...
class Text
  ...
  def decode_string_hod(string)
    ...
    decoded_string = string.unpack("v*").map do |char|
      ...
      char = if HOD_SPANISH_CHARACTERS.key?(halfword)
        curr_is_command = false
        HOD_SPANISH_CHARACTERS[halfword]
      elsif multipart
        curr_is_command = true
  ...
  def encode_char_hod(input_char)
    if HOD_SPANISH_CHARACTERS.value?(input_char)
      return [HOD_SPANISH_CHARACTERS.invert[input_char]].pack("v")
    end
